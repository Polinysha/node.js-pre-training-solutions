...
describe('TodoService', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        service = new todo_service_1.TodoService(mockTodoApi);
    });
    // Тесты для create
    ...
    test('toggleStatus should change status from PENDING to COMPLETED', async () => {
        ...
        mockTodoApi.update.mockResolvedValue({ ...todo, status: types_1.TodoStatus.COMPLETED });
        const result = await service.toggleStatus(1);
        expect(mockTodoApi.update).toHaveBeenCalledWith(1, { status: types_1.TodoStatus.COMPLETED });
        expect(result.status).toBe(types_1.TodoStatus.COMPLETED);
    });
    test('toggleStatus should change status from COMPLETED to PENDING', async () => {
        ...
        mockTodoApi.getAll.mockResolvedValue([todo]);
        mockTodoApi.update.mockResolvedValue({ ...todo, status: types_1.TodoStatus.PENDING });
        const result = await service.toggleStatus(1);
        expect(mockTodoApi.update).toHaveBeenCalledWith(1, { status: types_1.TodoStatus.PENDING });
        expect(result.status).toBe(types_1.TodoStatus.PENDING);
    });
